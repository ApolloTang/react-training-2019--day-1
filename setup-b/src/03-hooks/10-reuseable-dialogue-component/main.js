...
const api = {
  getReddit: (subject) => {
    ...
    //   fetch(`https://www.reddit.com/r/${subject}.json`).then(
    //     response => response.json()
    //   ).then(json=>rs(json))
    // })

    return new CancelablePromise(rs=>{
      const json = {foo:subject}
...
const InfoDialogue = ({subject}) => {
    ...
    const Dialogue = () => {
      ...
      // ------------------------
        React.useEffect(()=>{
          ...
          const getReddit = api.getReddit(subject)
  ...
  return (
    <div>
      <button onClick={openDialogue} disabled={isOpen}>Open info {subject}</button>
      {isOpen && <Dialogue/> }
    </div>
  )
}
...
const MyApp = () => (
  <div>
    <InfoDialogue subject="reactjs"/>
    <InfoDialogue subject="frontend"/>
</div>
)
